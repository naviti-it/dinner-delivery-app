...
const showCart = () =>{
  ...
  useEffect(()=>{
    let totalPrice = cartItems.reduce(function(accumulator, item){
      return accumulator += (item.qty * item.price)}, 0)
      setTot(totalPrice)
  },[tot, flag, cartItems])
  ...
  return (
      ...
      className=' z-[101] fixed top-0 right-0 w-full md:w-375 h-screen bg-white drop-shadow-md flex flex-col'>
          <div className='w-full flex items-center justify-between p-4 cursor-pointer'>
              ...
              <motion.p whileTap={{ scale: 0.75 }} className='flex items-centr gap-2 p-1 px-2 my-2 bg-gray-100 rounded-md hover:shadow-md cursor-pointer text-textColor text-base' onClick={clearCart}>
                  Clear <RiRefreshFill />
              </motion.p>
          </div>
          {/* bottom section */}
