...
const CreateContainer = () => {
  ...
  const uploadImage = (e) => {
    ...
    uploadTask.on('state_changed', (snapshot)=>{
      ...
      console.log(error);
      setFields(true);
      setMsg('Error while uploaded : Try Againp ');
  ...
  const saveDetails = () => {
    ...
    try {
      if ((!title || !calories || !price || !imageAsset || !category)) {
        setFields(true);
        setMsg("Requared fields can't be empty ");
        setAlertStatus('danger');
        setTimeout(() => {
          setFields(false);
          setIsLoading(false);
    ...
    <div className='w-full min-h-screen flex items-center justify-center'>
      <div className='w-[90%] md:w-[75%] border border-gray-300 rounded-lg p-4 flex flex-col                       items-center justify-center gap-4'>
        ...
        <div className='w-full'>
          <select onChange={(e) => setCategory(e.target.value)} className='outline-none w-full text-base border-b-2 border-gray-200 p-2 rounded-md cursor-pointer bg-white'>
            <option
              value='other'
            >
              Select Category
